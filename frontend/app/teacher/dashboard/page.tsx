...
export default function TeacherDashboard() {
  ...
  if (isLoggedIn === null || loading) {
    return (
      <div className="flex items-center justify-center min-h-screen min-h-screen relative">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-primary mx-auto mb-4"></div>
          <p className="text-xl text-foreground/90 font-medium">Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen min-h-screen relative">
        ...
        <div className="px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <button 
                className="lg:hidden p-2 rounded-lg glass-card/5 hover:glass-card/10 transition-colors"
                onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                aria-label="Toggle menu"
              >
                {mobileMenuOpen ? <X className="w-6 h-6 text-foreground/90" /> : <Menu className="w-6 h-6 text-foreground/90" />}
              </button>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground tracking-tight">Teacher Dashboard</h1>
                  <p className="text-muted-foreground text-sm font-medium">Welcome back, {teacherName}</p>
                  {employeeId && <p className="text-muted-foreground/70 text-xs">ID: {employeeId}</p>}
                </div>
              </div>
            </div>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden glass-card border-b border-white/10 shadow-lg">
          <div className="px-4 sm:px-6 py-4">
              ...
              {teacherMenuItems.map((item, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-foreground/90 font-semibold text-sm">{item.title}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-12 text-center">
            <div className="inline-flex items-center gap-3 mb-4">
              ...
              <span className="text-primary font-semibold text-sm uppercase tracking-wider">Teacher Dashboard</span>
              <div className="w-2 h-2 bg-primary/100 rounded-full animate-pulse"></div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {teacherMenuItems.map((item, idx) => (
              <div
                ...
                className={`relative p-6 rounded-2xl border-2 transition-all duration-500 cursor-pointer group overflow-hidden glass-card hover:shadow-xl ${
                  item.borderColor
                } ${
