...
export default function StudentRegistrationForm() {
  ...
  const handlePhotoCapture = async (images: string[]) => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:5001/api/register-student", {
        method: "POST",
        headers: { 
    ...
    return (
      <div className="flex items-center justify-center w-full h-screen min-h-screen relative">
        <div className="text-center">
          ...
          <p className="text-2xl text-foreground/90 font-semibold">Checking access...</p>
          <p className="text-muted-foreground/70 mt-2">Please wait while we verify your permissions</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 relative overflow-hidden">
      ...
      {/* Header */}
      <header className="glass-card/90 backdrop-blur-xl border-b-2 border-white/10 shadow-lg relative z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className={`w-6 h-1.5 rounded-full transition-all duration-500 ${step >= 1 ? 'bg-primary/100' : 'glass-card/10'}`}></div>
                <div className={`w-6 h-1.5 rounded-full transition-all duration-500 ${step >= 2 ? 'bg-primary/100' : 'glass-card/10'}`}></div>
              </div>
            </div>
          </div>
          <button
          ...
          {step === 1 ? (
            <div className="glass-card/90 backdrop-blur-lg rounded-2xl p-6 border-2 border-white/10 shadow-2xl">
              ...
              <div className="text-center mb-6">
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r glass-card border-2 border-primary/20 rounded-xl mb-3">
                  <User className="w-5 h-5 text-primary" />
                  <span className="text-primary font-semibold">Student Information</span>
                </div>
                <h2 className="text-3xl font-bold text-primary mb-2">Register New Student</h2>
              ...
              <form onSubmit={handleFormSubmit}>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gradient-to-br glass-card rounded-xl p-6 border-2 border-primary/20 shadow-lg hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200 shadow-lg hover:shadow-xl transition-all duration-300">
                    <div className="flex items-center gap-3 mb-5">
                      ...
                      <h3 className="text-xl font-bold text-primary">Academic Details</h3>
                    </div>
                    <div className="space-y-4">
            ...
            <div className="glass-card/90 backdrop-blur-lg rounded-2xl p-6 border-2 border-white/10 shadow-2xl">
              <div className="text-center mb-6">
                ...
                <h2 className="text-3xl font-bold text-primary mb-2">Face Recognition Setup</h2>
                <p className="text-muted-foreground text-base leading-relaxed">
...
const InputField: React.FC<InputProps> = ({ name, value, onChange, icon: Icon, label, placeholder, type = "text" }) => (
  <div className="group">
    <label className="block text-foreground/90 text-sm font-semibold mb-2 transition-all duration-300 group-focus-within:text-primary">
      {label}
    </label>
    <div className="relative">
      <input
        ...
        className="w-full px-4 py-3 pl-12 rounded-lg glass-card border-2 border-white/10 text-foreground placeholder-slate-400 focus:outline-none focus:border-primary focus:ring-2 focus:ring-blue-100 transition-all duration-300 font-medium hover:border-slate-300 shadow-sm hover:shadow-md"
      />
      <div className="absolute left-3 top-1/2 transform -translate-y-1/2 p-1 rounded-md glass-card/5 group-focus-within:bg-primary/20 transition-all duration-300">
        <Icon className="w-4 h-4 text-muted-foreground/70 group-focus-within:text-primary transition-colors duration-300" />
      </div>
    </div>
  </div>
);
...
const SelectField: React.FC<SelectProps> = ({ name, value, onChange, icon: Icon, label, options, placeholder, prefix }) => (
  <div className="group">
    <label className="block text-foreground/90 text-sm font-semibold mb-2 transition-all duration-300 group-focus-within:text-emerald-600">
      {label}
    </label>
    <div className="relative">
      <select
        ...
        value={value}
        onChange={onChange}
        className="w-full px-4 py-3 pl-12 rounded-lg glass-card border-2 border-white/10 text-foreground focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all duration-300 font-medium hover:border-slate-300 shadow-sm hover:shadow-md appearance-none cursor-pointer"
      >
        <option value="" className="text-slate-400">{placeholder || `Select ${label}`}</option>
        {options.map(opt => (
          <option key={opt} value={opt} className="text-foreground font-medium">
            {prefix ? `${prefix}${opt}` : opt}
          </option>
        ))}
      </select>
      <div className="absolute left-3 top-1/2 transform -translate-y-1/2 p-1 rounded-md glass-card/5 group-focus-within:bg-emerald-100 transition-all duration-300">
        <Icon className="w-4 h-4 text-muted-foreground/70 group-focus-within:text-emerald-600 transition-colors duration-300" />
      </div>
