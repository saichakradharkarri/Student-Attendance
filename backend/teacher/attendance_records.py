...
# Safe lazy imports — degrade gracefully in mock/test environments
try:
    ...
    def ObjectId(s):
        try:
            ...
            return s  # Return raw string if not a valid ObjectId
except ImportError:
    def ObjectId(s):
        return s  # No-op when bson not available
...
def detect_faces_optimized(rgb_image, detector):
    ...
    for d in detections:
        # Lowered threshold to 0.65 to support darker environments
